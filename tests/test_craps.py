...
class TestCrapsGame(unittest.IsolatedAsyncioTestCase):
    async def test_craps_win_rate(self):
        ...
        with patch("craps.get_user_data", new_callable=AsyncMock) as mock_get_data, \
             ...
             patch("craps.is_frontman", new_callable=AsyncMock) as mock_fm, \
            ...
            mock_update.side_effect = [900, 1100]

            await craps.process_craps_confirm(callback, state)
    ...
    async def test_craps_loss_rate(self):
        ...
        with patch("craps.get_user_data", new_callable=AsyncMock) as mock_get_data, \
            ...
            mock_update.return_value = 900

            await craps.process_craps_confirm(callback, state)
            mock_update.assert_called_once_with(123, 456, -100, min_balance=-5000)
